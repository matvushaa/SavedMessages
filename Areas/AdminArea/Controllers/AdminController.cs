...
namespace SavedMessages.Areas.AdminArea.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public AdminController(IManager<Message> mesmanager, IManager<Sticker> smanager,
        ...
        [HttpPost]
        public ActionResult DeleteMessage(int id, object o)
        {
            try
            {
        ...
        public ActionResult EditMessage(int id, Message message)
        {
            if (DateTime.Now <= message.Time.AddMinutes(15.0))
            {
                try
                {
                    ...
                    return RedirectToAction("GetSavedMessages");
                }
                catch
                {
                    return RedirectToAction("Index");
        ...
        [HttpGet]
        public ActionResult DeleteFromSaved(int id)
        {
            return View(messagemanager.GetById(id));
        }

        [HttpPost]
        public ActionResult DeleteFromSaved(int id, Message message)
        {
            if (DateTime.Now <= message.Time.AddMinutes(15.0))
             { 
                try
        ...
        [HttpGet]
        public ActionResult GetSavedMessages()
        {
            return View(messagemanager.GetAll().Where(x => x.IsSaved == true).Where(p=>p.UserId==myguid));
        }

        [HttpGet]
        public ActionResult SaveMessage(int id)
        {
            return View(messagemanager.GetById(id));
        }

        [HttpPost]
        public ActionResult SaveMessage(int id,Message message)
        {
            message.UserId = myguid;
            message.IsSaved = true;
            messagemanager.Edit(id,message);
